...
internal class DoujinDesuParser(context: MangaLoaderContext) :
		...
		return webClient.httpGet(url).parseHtml()
			...
			.mapNotNull {
				...
				Manga(
					...
					rating = it.selectFirst(".metadata .score")?.text()?.trim()?.toFloatOrNull()?.div(10f) ?: RATING_UNKNOWN,
	...
	override suspend fun getDetails(manga: Manga): Manga {
		...
		return manga.copy(
			...
			description = docs.selectFirst(".metadata .pb-2")?.selectFirst("p")?.html(),
			state = state,
	...
	override suspend fun resolveLink(resolver: LinkResolver, link: HttpUrl): Manga? {
		...
		if (segments.size >= 2 && segments[0] == "manga") {
			val slug = segments[1]
			if (slug.isNotBlank()) {
				return resolver.resolveManga(this, "/manga/$slug/")
			}
		}
		return null
	}
